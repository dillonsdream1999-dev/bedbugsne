...
export type VideoFirstPpcLandingContent = {
  canonical: string;
  ...
  recapTitle?: string;
  recapBullets?: string[];
  postVideoHeadline?: string;
  postVideoSubtext?: string;
  whyTitle?: string;
...
const whyCards = [
  ...
  },
  {
    ...
    body: "The goal is more than knocking down visible bugs. Proper heat reaches lethal temperatures for bed bugs and eggs.",
  },
  {
...
const faqs = [
  {
    ...
    answer:
      "Yes. Proper professional heat reaches lethal temperatures for bed bugs and eggs. We monitor the treatment so heat reaches hiding places, not just open rooms.",
  },
  {
  ...
  {
    question: "Can I finance treatment?",
    answer:
      "Financing is available through American First Finance for qualified customers. The application is linked from this page.",
  },
];
...
function EstimateForm({ submissionSource, title, description }: { submissionSource: string; title?: string; description?: string }) {
  return (
    <form action={formspreeEndpoint} method="post" className="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl sm:p-7">
      ...
      <input type="hidden" name="source" value={submissionSource} />
      <div className="mb-5 text-center">
        <p className="text-sm font-bold uppercase tracking-wide text-amber-600">Prefer a callback?</p>
        <h2 className="mt-2 text-3xl font-bold tracking-tight text-gray-900">{title ?? "Request a free heat treatment estimate"}</h2>
        <p className="mt-3 text-gray-600">{description ?? "Short form. We need your name and phone number first. Add details if you want."}</p>
      </div>
...
function JsonLd({ content }: { content: VideoFirstPpcLandingContent }) {
  const data = [
    {
      "@context": "https://schema.org",
      "@type": "BreadcrumbList",
      itemListElement: [
        { "@type": "ListItem", position: 1, name: "Home", item: "https://bedbugsne.com/" },
        { "@type": "ListItem", position: 2, name: "Landing", item: "https://bedbugsne.com/landing/" },
        { "@type": "ListItem", position: 3, name: content.breadcrumbName, item: content.canonical },
      ],
    },
    {
      "@context": "https://schema.org",
      "@type": "PestControl",
      name: "Midwest Bed Bug Services Nebraska",
      url: content.canonical,
      telephone: "+14025098890",
      areaServed: ["Omaha", "Lincoln", "Council Bluffs", "Nebraska", "Western Iowa"],
    ...
    <main className="bg-white pb-24 md:pb-0">
      ...
      <section className="relative overflow-hidden bg-[#173456] pt-24 text-white sm:pt-28">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-10 sm:px-6 md:py-16 lg:px-8 lg:py-20">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div>
              <p className="mb-3 text-sm font-bold uppercase tracking-[0.22em] text-amber-300">{content.eyebrow}</p>
              <h1 className="text-4xl font-bold leading-tight md:text-5xl lg:text-6xl">{content.title}</h1>
              <p className="mt-5 text-xl leading-relaxed text-blue-100">{content.subhead}</p>
            ...
            <div id="watch-video" className="scroll-mt-28" data-cta-section="video-hero">
              <div className="mb-3 flex items-center justify-between gap-3">
                <p className="text-sm font-bold uppercase tracking-wide text-amber-300">{content.videoLabel}</p>
      ...
      <section className="bg-white py-12 md:py-16">
        <div className="mx-auto grid max-w-5xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.8fr_1fr] lg:items-start lg:px-8">
          <div>
            <p className="text-sm font-bold uppercase tracking-wide text-amber-600">{content.recapEyebrow ?? "Can’t watch right now?"}</p>
            <h2 className="mt-2 text-3xl font-bold text-gray-900 md:text-4xl">{content.recapTitle ?? "Here’s the short version."}</h2>
          </div>
      ...
      <section className="bg-slate-50 py-10 md:py-14">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8" data-cta-section="post-video-panel">
          <div className="rounded-3xl bg-blue-900 p-6 text-white shadow-xl md:p-10">
            <h2 className="text-3xl font-bold">{content.postVideoHeadline ?? "Want to know if heat treatment is right for your home?"}</h2>
            <p className="mt-3 text-lg text-blue-100">{content.postVideoSubtext ?? "Tell us what you’re seeing. We’ll tell you what to do next."}</p>
      ...
      <section id="why-heat" className="bg-white py-14 md:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto mb-10 max-w-3xl text-center">
            <h2 className="text-3xl font-bold text-gray-900 md:text-4xl">{content.whyTitle ?? "Why homeowners call us instead of spraying again"}</h2>
            <p className="mt-4 text-lg text-gray-600">{content.whyDescription ?? "Bed bugs are hard to beat with partial fixes. Heat is for people who want the home treated completely."}</p>
          </div>
      ...
      <section className="bg-slate-50 py-14 md:py-20">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[1fr_0.9fr] lg:items-center">
            <div>
              <p className="text-sm font-bold uppercase tracking-wide text-amber-600">Prep and payment</p>
              <h2 className="mt-2 text-3xl font-bold text-gray-900 md:text-4xl">Know what happens before you schedule</h2>
              <p className="mt-4 text-lg leading-relaxed text-gray-600">
                We explain prep before treatment and give you a clear estimate before you book. Financing is available for qualified customers.
              </p>
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                <a className="inline-flex min-h-12 items-center justify-center rounded-xl bg-blue-900 px-5 py-3 text-base font-bold text-white hover:bg-blue-800" href="/downloads/mwbbs_heat_treatment_prep.pdf">
                  Download prep checklist
                </a>
                <a className="inline-flex min-h-12 items-center justify-center rounded-xl border border-blue-200 bg-white px-5 py-3 text-base font-bold text-blue-900 hover:bg-blue-50" href={financeUrl}>
                  Apply for financing
                </a>
              </div>
            </div>
            <Image
              ...
              alt="Bed bug heat treatment diagram"
              width={900}
              height={650}
      ...
      <section id="free-estimate" className="scroll-mt-28 bg-white py-14 md:py-20">
        <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
          <EstimateForm submissionSource={content.submissionSource} title={content.formTitle} description={content.formDescription} />
        </div>
      </section>

      <section className="bg-slate-50 py-14 md:py-20">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-center text-3xl font-bold text-gray-900 md:text-4xl">{content.faqTitle ?? "Bed bug heat treatment questions"}</h2>
          <p className="mb-8 mt-3 text-center text-gray-600">{content.faqDescription ?? "The main things people ask before they call or text."}</p>
            ...
            {faqs.map((faq) => (
              <details key={faq.question} className="group p-5 open:bg-slate-50">
                <summary className="cursor-pointer list-none text-lg font-bold text-gray-900">
                  {faq.question}
                  <span className="float-right text-amber-600 group-open:rotate-45">+</span>
                </summary>
      ...
      <section className="bg-[#173456] py-14 text-white md:py-20">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8" data-cta-section="final-video-landing-cta">
          <h2 className="text-3xl font-bold md:text-4xl">{content.finalTitle ?? "Ready to talk through your home?"}</h2>
          <p className="mt-4 text-lg text-blue-100">{content.finalDescription ?? "Call or text now. We’ll help you figure out the next step."}</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <CtaButton href={phoneHref}>Call {phoneDisplay}</CtaButton>
            <CtaButton href={smsHref} variant="secondary">Text a question</CtaButton>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-200 bg-white p-3 shadow-2xl md:hidden">
        <div className="grid grid-cols-2 gap-3">
          <a className="rounded-xl bg-amber-500 px-4 py-3 text-center text-sm font-bold text-white" href={phoneHref}>Call now</a>
          <a className="rounded-xl bg-blue-900 px-4 py-3 text-center text-sm font-bold text-white" href="#free-estimate">Free estimate</a>
        </div>
      </div>
    </main>
  );
